...
- Local Only
    ...
    - Some future features may require an internet connection (such as NTP time or to auto-fetch images?), but this firmware shall be designed to require explicit permission to connect to the internet and the ability to work without it
- No microphone usage
...
- No AI Image Content
    - I personally despise AI generated illustrations, thus I shall not enable it. While I can't, nor should, be able to disallow you to upload images generated by AI, such direct integration in this firmware will not be allowed.
...
To run it, you first must build the firmware like normal with the idf sourced, then run the make file. Right now only 1 target exists, `make test_apis`.
...
- Rev 0.1 (WIP)
    ...
    - Functionality includes:
        -[x] Basic Functionality (the display)
        -[x] SD Card image loading/storing
        -[x] Image ~~cycle~~ playlist mode
        -[x] HTTP API
        -[x] PMIC reading
- Rev 0.2 (PLANNED)
...
While I do use ChatGPT in the my development, it is used as a pointer to information that I may not have otherwise known, and to generate code blocks. That said, the code it generates is not blindly copied-and-pasted without understanding it's function, and in most cases I write it out myself and look up the API docs manually for a full understanding.

I will not accept purely AI generated code without understanding of the code, especially AI bot-generated PRs. The author must have full understanding of their code.
